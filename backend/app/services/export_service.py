...
class ExportService:
        ...
        if session.data:
            ...
            if session.data.network_errors:
                ...
                for err in session.data.network_errors[:5]:
                    lines.append(
                        f"- {err.get('method', 'GET')} {err.get('url', '')[:80]} → {err.get('status', '?')}"
                    )
                lines.append("")
    ...
    def export_postman(self, session: Session) -> dict:
        ...
        if session.data and session.data.recorded_requests:
            ...
            requests = [RecordedRequest(**req) for req in session.data.recorded_requests]
    ...
    def export_pytest(self, session: Session) -> str:
        ...
        exchanges = [RecordedHttpExchange(**req) for req in session.data.recorded_requests]
