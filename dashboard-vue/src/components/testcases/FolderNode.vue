<template>
  <div>
    ...
    >
      ...
      <span v-if="folder.test_cases_count" class="folder-count">{{ folder.test_cases_count }}</span>
    </div>

    <div v-if="expanded" class="folder-children">
      <!-- Nested test cases -->
      <div
        v-for="tc in folder.test_cases"
        :key="tc.id"
        class="tree-item testcase-item"
      ...
      >
        <span class="tree-icon">&#128203;</span>
        <span class="testcase-name">{{ tc.title }}</span>
      </div>
...
const hasChildren = computed(() =>
  (props.folder.children?.length > 0) || (props.folder.test_cases?.length > 0)
)
